...
def myGenerator(y, batch_size, fnames):
	...
	file_shape = sample_file.shape[1:]
	...
	while True:
		...
		for i in xrange(np.ceil(1.0*len(fnames)/batch_size).astype(int)):
			...
			X = np.zeros(((this_batch_size,) + file_shape)).astype('float32')
			...
			'''
			X = pool.map(filereader, fnames[i*batch_size:i*batch_size+this_batch_size])
			X = np.array(X).reshape((this_batch_size, new_img_colours, new_img_rows, new_img_cols)) #.astype('float32')
			'''
			
			
			i2 = 0 
			for temp_array in pool.imap(filereader, fnames[i*batch_size:i*batch_size+this_batch_size]):
				X[i2] = temp_array
				i2 += 1
